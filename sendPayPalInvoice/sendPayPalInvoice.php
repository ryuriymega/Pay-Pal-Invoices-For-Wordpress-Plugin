 ...
 * Plugin URI:        https://linkedin.com/in/iurii-paimurzin
 * Version:           1.0.0
 * Author:            IURII PAIMURZIN
 * Author URI:        https://linkedin.com/in/iurii-paimurzin
...
if (!defined('MYPLUGIN_NAME_1'))
    define('MYPLUGIN_NAME_1', 'sendPayPalInvoice');
...
final class sendPayPalInvoice {
	...
	public function __construct() {
		add_action( 'init', array( $this, MYPLUGIN_NAME_1 ), -1 );
	...
	public function sendPayPalInvoice() {
		add_action( 'wp_enqueue_scripts', array( $this, MYPLUGIN_NAME_1.'_css' ), 999 );
		add_action( 'wp_enqueue_scripts', array( $this, MYPLUGIN_NAME_1.'_js' ) );
		add_filter( 'template_include',   array( $this, MYPLUGIN_NAME_1.'_template' ), 11 );
		add_filter( 'wc_get_template',    array( $this, MYPLUGIN_NAME_1.'_wc_get_template' ), 11, 5 );
	}
	...
	public function sendPayPalInvoice_css() {
		wp_enqueue_style( 'custom-css', plugins_url( '/'.MYPLUGIN_NAME_1.'/style.css', __FILE__ ) );
	}
	...
	public function sendPayPalInvoice_js() {
		wp_enqueue_script( 'custom-js', plugins_url( '/'.MYPLUGIN_NAME_1.'/custom.js', __FILE__ ), array( 'jquery' ) );
	}
	...
	public function sendPayPalInvoice_template( $template ) {
		if ( file_exists( untrailingslashit( plugin_dir_path( __FILE__ ) ) . '/'.MYPLUGIN_NAME_1.'/templates/' . basename( $template ) ) ) {
			$template = untrailingslashit( plugin_dir_path( __FILE__ ) ) . '/'.MYPLUGIN_NAME_1.'/templates/' . basename( $template );
		}
	...
	public function sendPayPalInvoice_wc_get_template( $located, $template_name, $args, $template_path, $default_path ) {
		$plugin_template_path = untrailingslashit( plugin_dir_path( __FILE__ ) ) . '/'.MYPLUGIN_NAME_1.'/templates/woocommerce/' . $template_name;
 ...
 */
add_action( 'plugins_loaded', MYPLUGIN_NAME_1.'_main' );
